...
render d = rend 0 (map ($ "") $ d []) "" where
  rend i ss = case ss of
    ...
    "}" : ";" : ts ->
      new (i - 1) . space "}" . showChar ';' . new (i - 1) . rend (i - 1) ts
  ...
  space t s = case (all isSpace t', null spc, null rest) of
...
instance Print a => Print (Def a) where
  prt i e = case e of
    DEquation a id pats exp -> prPrec
      i
      0
      (concatD
        [ prt 0 id
    ...
    DTypeSig id type_ ->
      prPrec i 0 (concatD [prt 0 id, doc (showString ":"), prt 0 type_])
    DDataDec uident ids constructordecs -> prPrec
      i
      0
      (concatD
        [ doc (showString "data")
...
instance Print (UIdent, Type) where
  prt i (uid, t) =
    prPrec i 0 (concatD [prt 0 uid, doc (showString ":"), prt 0 t])
  prtList _ []       = concatD []
  prtList _ [x     ] = concatD [prt 0 x]
  prtList _ (x : xs) = concatD [prt 0 x, doc (showString ";"), prt 0 xs]
...
instance Print Type where
  prt i e = case e of
    TLam type_1 type_2 ->
      prPrec i 0 (concatD [prt 1 type_1, doc (showString "->"), prt 0 type_2])
    TVar id -> prPrec i 1 (concatD [prt 0 id])
    TTup ts -> prPrec
      i
      1
      (concatD
        ([doc (showString "(")] ++ printTups ts ++ [doc (showString ")")])
      )
   ...
   where
    ...
    printTups [x] = [prt 0 x]
    printTups (x : y : xs) =
      [prt 0 x, doc (showString ",")] ++ printTups (y : xs)
  prtList n []       = concatD []
...
instance Print a => Print (Pat a, Exp a) where
  prt i (pat, exp) =
    prPrec i 0 (concatD [prt 0 pat, doc (showString "->"), prt 0 exp])
  prtList _ [x     ] = concatD [prt 0 x]
  prtList _ (x : xs) = concatD [prt 0 x, doc (showString ";"), prt 0 xs]
...
instance Print (Pat a) where
  prt i e = case e of
    ...
    PAs _ id pat ->
      prPrec i 2 (concatD [prt 0 id, doc (showString "as"), prt 0 pat])
    PAdt _ uident []      -> prPrec i 0 (concatD [prt 0 uident])
    PAdt _ uident adtpats -> prPrec
      i
      0
      (concatD
        [ doc (showString "(")
