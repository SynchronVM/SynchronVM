...
lookupBinop op = case Map.lookup op binoptypes of
  Just schema -> instantiate schema
...
moreGeneralThan (TTup ts1) (TTup ts2) = or $ zipWith moreGeneralThan ts1 ts2
...
checkTooGeneralType fun (Just sig) t = do
 ...
 where
  ...
  renameTVars t1 t2 = case (t1, t2) of
    ...
    (TLam t1 t2, TLam t1' t2') ->
      TLam (renameTVars t1 t1') (renameTVars t2 t2')
    (_, t) -> t
...
unifyWithTypesig fun (Just sig) t = do
  catchError (unify sig t >> return ())
    $ \_ -> throwError $ TypesigError fun sig t
...
checkPat p = case p of
  ...
  PConst () l -> return $ PConst (constType l) l
  PWild ()    -> do
    t <- fresh
    return $ PWild t
  PAs () id p -> do
...
checkExp e = case e of
  ...
  EBin () e1 e2 o -> do
    ...
    return
      (sub, EBin (apply sub tv) e1' e2' (setBinopVar (apply sub inftype) o))
  ...
  ELet () p e1 e2 -> do
    ...
    let env'' = foldl (\e' (id, sc) -> extend (restrict e' id) id sc)
                      env'
                      varschemas
...
inferPrim l ts typ = do
 ...
 where
  ...
  inferStep
    :: (Subst, Env, [Exp Type]) -> (Exp (), Type) -> TC (Subst, Env, [Exp Type])
...
checkCaseClauses
  :: Type -> [(Pat (), Exp ())] -> TC (Subst, [(Pat Type, Exp Type)])
...
checkProgram p = do
  ...
  foldM
    (\acc d -> do
      withConstructors acc $ checkDataDeclaration d
      return $ d : acc
    )
    []
    (datatypes p)
  ...
  let main' = last funs
  let funs' = init funs
  return (sub, p { functions = funs', main = main' })
