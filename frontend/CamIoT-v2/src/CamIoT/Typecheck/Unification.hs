...
unify (TAdt uid1 ts1) (TAdt uid2 ts2) | uid1 == uid2 = unifyMany ts1 ts2
...
bind id t | t == TVar id          = return unitsub
...
unifyWithAtleastOne t1 (t2 : ts) =
  catchError (unify t1 t2) $ \_ -> unifyWithAtleastOne t1 ts
